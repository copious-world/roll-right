...
console.log(a_var)
        ...
        while ( depth > 0 ) {
            ...
            if ( a_var.startsWith('#each') ) {
                ...
                if ( has_subs ) {
                    let sub_vars = {}
                    i = handle_outer_loop(var_starts,i,a_var,sub_vars)
                    loop_vars[control_var][a_var] = sub_vars
                } else {
                    i = handle_outer_loop(var_starts,i,a_var,found_vars)
                }
            }
            ...
            if ( (avar[0] === '{') || (avar.indexOf(".") > 0) ) {
                ...
                if ( (typeof rec === "object") && subkeys.length ) {
                    ...
                    if ( namer.length && actor.length && file.length ) {
                        ...
                        subst_obj[base_var_name][actor] = {  // e.g. actor === 'content'
                            "file" : `./${found_name}.${ftype}` // a file path into the concern's static directory
                        }
                    }
                    //
                }
