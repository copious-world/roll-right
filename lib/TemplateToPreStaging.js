...
class TemplatesToPreStaging extends SkelToTemplate {
    ...
    /**
     * Looks for a substitution place holder that references the outter loop variable
     * as a member field
     * 
     * @param {string} a_var 
     * @param {number} i_start 
     * @param {Array} var_starts 
     * @returns {boolean}
     */
    find_loop_dependent_vars(a_var,i_start,var_starts) {
        let i = i_start;
        let n = var_starts.length
        while ( i < n ) {
            let vstart = var_starts[i++]
            if ( vstart.startsWith("/each") ) {
                break
            }
            if ( vstart.startsWith(`${a_var}.`) ) return true
            if ( vstart.startsWith(`this.`) ) return true
        }
        return false
    }
     ...
     * 
     * 
     * In the recursion, `found vars` can be passed down, make data loops independent of each other.
     * Alternatively, a substucture of the loop collection variable can be passed down, requiring 
     * the data retreived by genpage.js to nest as dependencies to the outer loop.
     * 
     * 
     * @param {Array} var_starts 
     * @param {number} i 
     * @param {string} a_var 
     * @param {Object} found_vars 
     * @returns {number}
     */
    handle_outer_loop(var_starts,i,a_var,found_vars) {
        ...
        while ( true ) {
            ...
            if ( a_var.startsWith('#each') ) {
                // RECURSION  var_starts,i,a_var,found_vars
                let has_subs = this.find_loop_dependent_vars(a_var,i,var_starts)
                if ( has_subs ) {
                    let sub_vars = {}
                    i = this.handle_outer_loop(var_starts,i,a_var,sub_vars)
                    loop_vars[control_var][a_var] = sub_vars
                } else {
                    i = this.handle_outer_loop(var_starts,i,a_var,found_vars)
                }
            } else if ( a_var.startsWith('/each') ) {
                break
            } else {
                if ( a_var.indexOf('.' ) > 0 ) {
                    if ( a_var[0] === '{' ) {
    ...
    /**
     * 
     * Loads the top level file `concerns named DB`, which maps concerns to DB files associated with each template.
     * 
...
//
    ...
    ```
     * 
     * Example output derived from a db entry:
     * ```
    "contact_box": {                // contact_box is base_var_name
        "name": "contact",          // often this is the text of a hyperlink or ref to the content
        "contact_box": {
            "file": "./contact.@<type>"     // location of the file containing markup 
        }
    },
    ```
     * 
    ...
    get_subst_vars(concern,var_set,var_src_file) {
        ...
        for ( let avar in var_set ) {
            if ( (avar[0] === '{') || (avar.indexOf(".") > 0) ) {
                ...
                if ( subkeys && (subkeys.indexOf('content') >= 0) && (subkeys.indexOf('button') < 0) ) {
                    if( !(this.namer_in_name_db(concern,base_var_name,var_src_file)) ) {
                        subst_obj[base_var_name].file = ""
//console.log("content",base_var_name, subkeys)
                    }
                }
                if ( (typeof rec === "object") && subkeys.length ) {
                    ...
                    if ( namer.length && actor.length && file.length ) {
                        ...
                        if ( subst_obj[base_var_name] === undefined ) {
                            subst_obj[base_var_name] = {        // construct the subst entry needed to load this type
                                "name" : found_name
                            }
                        } else {
                            subst_obj[base_var_name].name = found_name
                        }
                        if ( subst_obj[base_var_name].content === undefined ) {
                            delete subst_obj[base_var_name].file
                            subst_obj[base_var_name].content = {  // e.g. actor === 'content'
                                "file" : `./${actor}.${ftype}` // a file path into the concern's static directory
                            }
                        }
                    }
                    //
    ...
    namer_in_name_db(concern,namer,file) {
        if ( file ) {
            ...
            for ( let ky of Object.keys(this.all_concerns_namer_db[concern]) ) {  // there is a db per page generated
                if ( ky.indexOf(file_prober) >= 0 ) {    // looking for the file that goes with the template being generated 
                    if ( namer in this.all_concerns_namer_db[concern][ky] ) {  // A field name in the db. For instance "topix_box_1"
                        return true
                    }
                }
            }
        ...
        if ( file ) {
            ...
            for ( let ky of Object.keys(this.all_concerns_namer_db[concern]) ) {
                if ( ky.indexOf(file_prober) > 0 ) {
                    if ( namer in this.all_concerns_namer_db[concern][ky] ) {   // e.g. "topicBox_2"
                        let defs = this.all_concerns_namer_db[concern][ky][namer] // names the file with content
                        return [defs.name, defs.content.type]   // the name of the file with content and the type of the file conent
                    }
                }
            }
        }
