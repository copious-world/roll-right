...
if ( testing ) console.log("||||----------the_file-------build_param_tree",the_file)
                        ...
                        // var_holder
                        rest = this.determine_element_generator(rest,var_holder,el_gen)
     ...
     * @returns {string}
     */
    determine_element_generator(rest,var_holder,el_gen) {
        let next = this.find_next(rest)
        let param_block = ""
        if ( next ) {
        ...
        let promisory = data_parts.map(async (a_part) => {  // map and return an array of promises
            if ( (a_part.indexOf("files::") === 0)  || (a_part.indexOf("script::") === 0)  ) { ///
                if ( (a_part.indexOf("files::params::") === 0)  || (a_part.indexOf("script::params::") === 0) ) {  // 
                    ...
                    //
let testing = false
if ( part_key === "files::params::flexy_items_B-main.smplt" ) {
    testing = false
    console.log("CAPTURE PARAMETERS",part_key)
    console.log("CAPTURE PARAMETERS >>",part_def.substring(0,part_def.lastIndexOf('}')))
//debugger;
}
                    //
                    maybe_params[part_key] = await this.build_param_tree(part_key,part_def.substring(0,part_def.lastIndexOf('}')),testing)
                    //
if ( testing ) {
console.dir(maybe_params[part_key],{ depth : 8})
}
                    return part_key
                }
            }
