...
let common_dir = path.dirname(path.dirname(this_file))



let SysUtils = require('../lib/sys_utils')
let sys_utils = new SysUtils({
    "tools_directory" : `${common_dir}/tools`
})
...
/*
Freedom of Movement for Human Agency and Identity: Information technology. Artificial Intelligence, IoT, and Songs
*/
...
class SkelToTemplate {
    ...
    constructor(conf) {
        ...
        this.links_and_bundles = conf.links_and_bundles ? conf.links_and_bundles : false
        ...
        this.free_list_pattern = new RegExp(/.*\<(\w+)\>/)
        //
        this.var_spec_pattern_files = new RegExp(/^\$\@\{(\w+)\}\$files\:\:(.*)$/)
        this.var_spec_pattern_scripts = new RegExp(/^\$\@\{(\w+)\}\$script\:\:(.*)$/)
        this.var_spec_pattern_values = new RegExp(/^\$\@\{(\w+)\}\$value$/)

        this.var_spec_copy_pattern = new RegExp(/^\$\@\{(\w+)\}\$verbatim$/)
        this.var_spec_copy_pattern_2 = new RegExp(/^\$\@\{(\w+)\}\$value$/)
...
// build_param_tree files::params::nav_bar_V.smplt $@{mushroom}$icons::mushroom-menu-icon.svg<<
        ...
        this.start_of_indexed_list = new RegExp(/^\@list\<(\w+)\>/)
        ...
        // the resultant file will have quotes. The quotes should be utilized during the final "handle-bars"
        // matching in order to retreive a file contents. The quotes are put in. There may be a case
        // where they go missing.
        this.generic_ogroup_target_dotted_path = new RegExp(/\{\{\{\s*(\"\@\{\[(\w+)\.(\w+)\]\}).*/)

        this.range_check_pattern = new RegExp(/\<.*,.*\>/)
    ...
    // {{{ "@{[targets.dir]}/@kernel/site_app.js" }}}
...
//console.dir(parts_holder)
            ...
            if ( substkey[0] === '\"' ) substkey = substkey.substring(1)
            //
     ...
     * @returns {string}
     */
    offset_from_project_dir(file_name) {
        let target_file =''
        if ( this.project_dir.indexOf("/home/") === 0 ) {
            target_file = `${this.project_dir}/${file_name}`
            target_file = target_file.replaceAll('//','/')
        } else {
            if ( this.project_dir[0] !== '[' ) {
                target_file = `[websites]/${this.project_dir}/${file_name}`
            }
            target_file = this.paths.compile_one_path(target_file)
        }
        return target_file
    }
    ...
    */
    async prepare_directories() {
        ...
        for ( let ogroup of outputs ) {
            let targets = ogroup.targets // targets
            let skeletons = ogroup.skeletons  // skeletons
            ogroup.saved_abbrev_skels = structuredClone(skeletons)
    ...
    /**
     * 
     * `uses_config_vars` is a map of the precompiled skel file map to an op name. 
     * The op name in turn references an `app_skel_vars_specials` map, mapping the op name 
     * to a set of specializations give for a concern meant to alter the skeleton for a 
     * particular page structure similar to the original skeleton. These specializations 
     * may influence section count, template database fillers, etc.
     * 
     * `uses_config_vars` is a map a website for a concern may use a collection of skeletons 
     * with one or more needing an "op" on a skeleton. The "op" must appear in `app_skel_vars_specials`
    ...
    */
    async load_skeletons() {
        ...
        for ( let ogroup of outputs ) {
            ...
            //
            // the entries are { "<output>.html" : "<skeleton file name>.skel"}
            for ( let [fky,file] of Object.entries(skeletons) ) {  // the field key is the desired output
                //
                let abbrev_path = ogroup.saved_abbrev_skels[fky]
                let fpath = this.paths.compile_one_path(file)
                let p = all_skeletons[fpath]            // a skeleton absolute path name
                if ( p === undefined ) {
                    all_skeletons[fpath] = {
                        "original" : "",
                        "ops" : {   // ops map -- these will be versions of the skeleton (with some overlap for different concerns using the same basic skeleton)
                        }
                    }
                }
                //
                if ( ogroup.skels_processed[abbrev_path] === undefined ) {     // one for any of the selected skeletons (may select in duplicate)
                    let c_vars_keys =  ogroup.targets.uses_config_vars  // `uses_config_vars` maps to `app_skel_vars`
                    let c_vars_key = (c_vars_keys ? c_vars_keys[abbrev_path] : false)
                    c_vars_key = (c_vars_key ? c_vars_key : "default")  // the skeleton ops apply to an ogroup
                    ogroup.skels_processed[abbrev_path] = {        // allows to map from template to output
                        "ablsolute_path" : fpath,           // a key to the loaded data as well
                        "encoded_path" : abbrev_path,       // back ref ... trying to avoid confusion
                        "skel_vars_key" : [c_vars_key],       // this is an op (one word)
                        "final" : ""        // final output not yet determined
                    }
                    // find .ops again in `section parsing`
                    if ( c_vars_key !== "default" ) {
                        all_skeletons[fpath].ops[c_vars_key] = structuredClone(this.app_skel_vars[c_vars_key])  // handle the generic case
                        // on preparation across many entries retrievable from ogroup by concern
                        let concerns = ogroup.targets.concerns
                        //
                        for ( let concern of concerns ) {
                            let specializations = ogroup.targets?.app_skel_vars_specials
                            if ( specializations && specializations[concern] ) {
                                let op_special = specializations[concern][c_vars_key]
                                let special_ky = `${c_vars_key}_${concern}`
                                let specialization = Object.assign(structuredClone(this.app_skel_vars[c_vars_key]),op_special)
                                all_skeletons[fpath].ops[special_ky] = specialization  // handle the generic case
                                ogroup.skels_processed[abbrev_path].skel_vars_key.push(special_ky)
                            }
                        }
                    } else {
                        all_skeletons[fpath].ops["default"] = false  // on preparation across many entries retrievable from ogroup by:
                    }
        ...
        //
        let data_promises = []                              // data loading promises
        let skel_keys = Object.keys(all_skeletons)
        for ( let fpath of skel_keys ) {
            data_promises.push(fos.load_data_at_path(fpath))
        }
        let data_list = await Promise.all(data_promises)    // resolve promises to list 
        let n = data_list.length
        //
        for ( let i = 0; i < n; i++ ) {
            all_skeletons[skel_keys[i]].original = data_list[i]     // this is before "ops" alteration
        }
        //
    ...
    has_free_list_expr(ctrl_str,var_holder) {
        let result = this.free_list_pattern.exec(ctrl_str)
        if ( result ) {
            var_holder[0] = result[1]
            return true
        }
    ...
    is_repetative_generator(key_part) {
        if ( key_part.indexOf("files::list") >= 0 ) return true
        if ( key_part.indexOf("files::loop") >= 0 ) return true
        if ( key_part.indexOf("script::list") >= 0 ) return true
        if ( key_part.indexOf("script::loop") >= 0 ) return true
        return false
    }
    ...
    /**
     * 
     * @param {string} the_file 
     * @returns {pair}
     */
    script_path_repair(the_file) {
        let path_finders = the_file.substring(the_file.indexOf('::[') + 3)
        path_finders = path_finders.split(']')
        let path_finder = path_finders[0].trim()
        the_file = path_finders[1].trim()
        if ( the_file[0] === '/' ) {
            the_file = the_file.substring(1)
        }
        if ( the_file.indexOf('<')  > 0 ) {
            the_file = the_file.substring(0,the_file.indexOf('<'))
        }
        return [path_finder,the_file]
    }
    ...
    async build_param_tree(part_key,param_str,testing) {
        //
        if ( param_str[0] === '{' ) {
        ...
        if ( sep_point > 0 ) {
            ...
            //
            do {
// if ( testing && ((key_part === "$@{list_name}$") || (key_part === "$@{topic_field}$") ) ) {
// if ( testing ) console.log("||||--------------------------build_param_tree",key_part,rest)
// }
// if ( testing ) console.trace()
                // The var spec pattern is the front of the line.
                // It indicates which var in the target parameterized file will be assigned 
                // the value result of the rest of the expression.
                let check = this.var_spec_pattern_files.exec(key_part)
                if ( !check ) {
                    check = this.var_spec_pattern_scripts.exec(key_part)
                }
                //
                if ( check ) {
                    ...
                    let var_holder = []
                    let el_gen = {}
                    // has_list_expr(the_file,var_holder) -- a condition for success is that this 
                    // expr should have already been converted to a numerical form, leaving no 
                    // dangling congiguration variables...
                    if ( this.is_repetative_generator(key_part) && this.has_free_list_expr(the_file,var_holder) ) { //

if ( testing ) console.log("||||----------the_file-------build_param_tree",the_file)

                        the_file = the_file.substring(0,the_file.indexOf('<'))
                        // var_holder
                        let var_name = var_holder[0]
                        rest = this.determine_element_generator(rest,var_name,el_gen)

                        sub_tree = false

                        // let sequence_spec = key_part.substring(key_part.indexOf("files::"))
                        // sequence_spec = `${sequence_spec}<<${el_gen.generator}`
                        // let data_parts = [ sequence_spec ]
                        // let maybe_params = {}
                        // data_parts = this.sequence_expansion(data_parts,maybe_params)
                        // el_gen.maybe_params = maybe_params
                    } else {
                        //
                        if ( rest.length ) {
                    ...
                    if ( the_file && parts_of_key.length > 1 ) {
                        let [path_finder,ftype] = ['','']
//console.log("build_param_tree",the_file)
                        if ( the_file.indexOf('::[') > 0 ) {
                            [path_finder,the_file] = this.script_path_repair(the_file)
                        } else {
                            [path_finder,ftype] = this.entry_directive_location_remap(parts_of_key[0])
                        }
                ...
                } else if ( (check = this.var_spec_copy_pattern.exec(key_part)) || (check = this.var_spec_copy_pattern_2.exec(key_part))  ) { //
                    //
                    let data =  ""
                    if ( rest.length ) {
                    ...
                    //
                } else if ( check = this.var_spec_value_pattern.exec(param_str) ) {  // this is what is expected with simple parameters
                    let data = check[2]
                    if ( rest.length ) {
                        let next = this.skip_value_next(rest)
                        if ( next ) {
                    ...
                    if ( check = this.var_spec_other_pattern.exec(key_part) ) {
                        ...
                        locus = this.top_dir_locations[path_finder]
    ...
    skip_value_next(rest) {
        ...
        if ( (rest.indexOf('${') >= 0) || (rest.indexOf('$@') >= 0) ) {
            ...
            return index
        } else if ( rest[0] === '@' && rest.startsWith('@!{') ) {
    ...
    async capture_param_sub_call(part_key,rest,sub_tree) {
        ...
        if ( next ) {
            ...
            rest = rest.substring(next)
            let a_tree = await this.build_param_tree(part_key,param_block,true)
    ...
    /**
     * 
     * @param {string} maybe_range 
     * @returns {boolean}
     */
    range_check(maybe_range) {
        let b = this.range_check_pattern.test(maybe_range)
        return b
    }
     ...
     * @returns {string}
     */
    determine_element_generator(rest,var_name,el_gen) {
        let next = this.find_next(rest)
        let param_block = ""
        if ( next ) {
            param_block = rest.substring(0,next)
            rest = rest.substring(next)
        } else {
            param_block = rest
            rest = ""
        }
        if ( (param_block[0] === "{") && (param_block[1] !== '\"') ) {
            param_block = param_block.substring(1,(param_block.lastIndexOf('}')))
        }
        el_gen.var_name = var_name
        el_gen.generator = param_block
        el_gen.is_range = this.range_check(param_block)
        //
        return rest        
    }
    ...
    /**
     * This is a top level syntax handler. In future versions, this structure should be folded into a 
     * recursive process. For lists, the data associated with the line is appeded to it and parse again
     * later. This state of affairs has to do with hurrying through a fist draft. At first, it wasn't clear
     * how much recursion would be needed in both parameter forms and sub file processing. It is now clear
     * that no limits should be put on subforms. Time allotted to this project has to do with this being 
     * handled for this verison.
     * 
            ...
            if ( a_part.indexOf("files::") === 0 ) {
                if ( a_part.indexOf("files::loop::") === 0 ) {
                    ...
                    if ( this.has_range_expr(a_part.substring("files::loop::".length),range) ) {
                        ...
                        return lines        // the reason for flatten below
                    } 
                } else if ( a_part.indexOf("files::list::") === 0 ) {
                    let var_holder = []
                    if ( this.has_list_expr(a_part.substring("files::list::".length),var_holder) ) {
                        // ELEMENTS
                        ...
                        let part_def = parts.shift()  // the data part that was deposited by config var expansion
                        let def = part_def
                        try {
    ...
    has_verbatim_params(a_part) {
        return (a_part.indexOf('$verbatim') > 0) || (a_part.indexOf('$value') > 0)
    }
        ...
        let promisory = data_parts.map(async (a_part) => {  // map and return an array of promises
            if ( (a_part.indexOf("files::") === 0)  || (a_part.indexOf("script::") === 0)  ) { ///
                if ( (a_part.indexOf("files::params::") === 0)  || (a_part.indexOf("script::params::") === 0) ) {  // 
                    ...
                    //
// if ( a_part.indexOf("/floating_window_controls.js") > 0 ) {
// console.log("capture_parameters",a_part)
// debugger;
// }
                    //
    ...
    /**
     * 
     * This replacement is being done on the entry. Later the data is pulled off and then looped 
     * or generated according to a formula.
     * 
     ...
     * @returns {Array}
     */
    config_var_transformations(data_parts,vars) {
        //
        if ( vars && (typeof vars === "object")) {
            for ( let [vname,value] of Object.entries(vars) ) {
                if ( typeof value === "object"&& !(Array.isArray(value)) ) {
                    if ( typeof value.$_type === "string" ) {
                        if ( "object<>" === value.$_type ) {
                            vars[vname] = value.$_name
                            for ( let fld in value ) {
                                if ( fld === "$_type" ) continue
                                if ( fld === "$_name" ) continue
                                vars[`${vname}.${fld}`] = value[fld]
                            }
                        }
                    }
                }
            ...
            for ( let [vname,value] of Object.entries(vars) ) {
                ...
                if ( typeof value !== "string" ) {
                    if ( Array.isArray(value) ) {
                        indexed_value = [].concat(value)
                        for ( let i = 0; i < value.length; i++ ) {
                            let iv = indexed_value[i]
                            if ( typeof iv === "object" ) {
                                iv.index = i+1
                            }
                        }
                    ...
                    indexed_value = JSON.stringify(indexed_value)
                }
                if ( value === undefined ) {
                    value = ""
                }
                ...
                data_parts = data_parts.map((el) => {       // alter the skeleton (an update)
                    ...
                    if ( el.indexOf(index_list_finder) > 0 ) {
                        el = el.replace(index_list_finder,indexed_value)
                    }
                    return el
                })
                //
     ...
     * 
     * 
     * @param {object} all_skeletons - A map of intialized skeleton file names to the data object carrying parsing and semantic information.
...
// if ( sky.indexOf("web3-mixed-boxy") > 0 ) {
// debugger
// }
            // 
            let data = data_m.original      // data_m is the data prepared in loading...
            // data_parts an array
            ...
            //
            section_data[sky] = {}
            let skel_ops = data_m.ops       // ascertained from ogroups during loading
            //
            data_parts.shift() // the first element is expected to be empty or white space or some comment (throw away)
            data_m.section_strings = data_parts    // add to data_m the header of the 
            //
            ...
            if ( def_check.indexOf("defs:") === 0 ) {  // if present, this becomes a structure.
                ...
                data_m.section_strings = data_parts
                //
                ...
                if ( def_defaults ) {       // defaults from the skeleton file $$defs
                    let cvars_ky = def_defaults?.uses_config_vars  // skeleton text :: a name found in a configuration
                    if ( typeof cvars_ky === "string" ) {
                        //
                        // use configuration variable values to put in values for skeleton level variables
                        // alter data parts 
                        data_parts = this.config_var_transformations(data_parts,skel_ops[cvars_ky])
                        ...
                        data_parts = dps
                        section_data[sky]["defaults"] = {  // different from "default" that is added based on the configuration
                            "defaults" : def_defaults,
                            "final" : {
                                "markup" : false,
                                "scripts" : false
                            },
                            "skeleton" : data_parts,        // only split at this point
                            "files" : files,
                            "css" : css,
                            "scripts" : scripts,
                            "bundles" : bundles,
            ...
            for ( let op_ky in skel_ops ) {  // skel_ops[cvars_ky]
                // construct this map, mapping the skeleton and operation 
                // to a compilation.
                if ( section_data[sky][op_ky] === undefined ) {  // the object will be made using the object skel_ops[cvars_ky]
                    //
                    data_parts = data_m.section_strings  // clean up data from start of outer loop
                    // 
                    // use configuration variable values to put in values for skeleton level variables
                    data_parts = this.config_var_transformations(data_parts,skel_ops[op_ky])
                    // next expand sequences and get separate imports for markup and script
                    let [dps,files,css,scripts,bundles,maybe_params] = await this.skeleton_parts_extraction(data_parts)
                    data_parts = dps
...
// process.exit(0)
     ...
     * 
     * Note that template content will use non-commital template variables such as `topicBox_1.content`.
     * The user edits his version of the DB to say where content comes from. It will stored in his static 
     * directory.
     *
     * `
        "name_parameters" : {
...
console.log("DATA")
                ...
                let script_grouping_dir = this.offset_from_project_dir(`bundle_src/${bare_ky}`)                
     ...
     * 
     * Called under script::, under params::, etc.
     * 
     * @param {string} step_entry 
...
//console.log(file_name,path_finder,entry_type,file_form,use_case,p_finder,data)
    ...
    index_list_start(data,carrier) {
        let dat_match = this.start_of_indexed_list.exec(data)
        if ( dat_match ) {
            carrier.var = dat_match[1]
            return true
        }
    ...
    async get_files_and_vars(step_entry,data) {
        ...
        if ( data.startsWith("@params<") ) {   // will be at the top of a file (parameter defs for matching calling skeleton)
            ...
            //
        } else if ( this.index_list_start(data,carrier) ) {
            _is_loop = true
            let var_name = carrier.var
            params_def = {
                "_var_name" : var_name
            }
            params_def[var_name] = `${var_name}[1]`
        } else {
    ...
    /**
{
    ...
    evaluation_map(params) {
        ...
        for ( let ev_pair of parlist ) {
            ...
            ky = ky.replace('}$',"")
            if ( (val.indexOf('@!{') === 0) || (val.indexOf('@#{') === 0)  || (val.indexOf('@#!{') === 0) ) { val = '0' }
            evals[ky] = val
        }
        return evals
    }
     ...
     * 
     * In the basic doc format below, the sheff stroke "|" is used as a meta separator.
     * The contents between the two separators describes a type, such as |the name of a file| or 
     * |the parameter variable goes here|. The higher level description is likely harder to read
     * than an example. The examples are shorter, as they would appear in a skeleton file.
     * 
     * 
     ...
     * $$files<*>::     -- These are files of a different type than the markup but in the same directory. For instance '*' may be replace with 'js' 
     ...
     * 
     * // Looping refered to as sequence expantions: see `sequence_expansion`
     * // Handled in looping... prior to this method
     * **Example**
     * $$files::list::MAP-sections_frame.smplt<el><<@!{groups}
     * **Abstract Form**
     * $$files::list::|file path|<|element identifier from file|><<{|element source|}
     * 
     * // Handle generating a list of elements with and index variable used for tagging
     * // the similar names, e.g. id="an_element_@{index}"
     * 
     * **Example**
     * $$files::loop::calc::INDEXED-sections_frame.smplt<index><<{<1,@!{section_count}>}
     * **Abstract Form**
     * $$files::loop::calc::|file path with an index variable taking values from a range|<|the variable martching
     * the index variable in the file'|><<${ a range, e.g. '<start,stop> or a calculation advancing the loop index }
     * 
     * 
     * $$files::|file path|<<
    ...
    async build_entry_parsing(sk_key,skeleton,skeleton_src,sk_map,incrementer_set,concern_rewrites) {
        ...
        for ( let step_entry of skeleton ) {
            ...
            // BUNDLE - a link definition (CDN connection perhaps)
                ...
                sk_map[`bundle::${hashed}`] = bundle_inclusion_transform(str,page_or_worker_context[skeleton_src],this.links_and_bundles)
            // LINK - a link definition (possibly with compression)
                ...
                let hashed = crypto.hash('sha1',str)        // cryp]==to
                sk_map[`link::${hashed}`] = link_inclusion_transform(str,page_or_worker_context[skeleton_src],this.links_and_bundles)
...
// console.log("DONE CALL --  ENTRY LOADING",step_entry)
                    ...
                    // TEMPLATE 
                        ...
                        data = data.substring(brace_i+1,brace_n-1)
...
//console.log("conditionless_evaluations_substitution", data)
                    ...
                    for ( let [entry_ky,desciptor] of Object.entries(skel.parameterized) ) {
                        ...
                        if ( parameter_key ) {
                            ...
                            for ( let par of pars ) {
                                ...
                                if ( pdescr === undefined ) {
                                    console.log(par)
                                    console.dir(desciptor)
                                    continue
                                }
                                if ( pdescr.element_generator ) {
                                    if ( !pdescr.conds ) continue
                                }
                                if ( typeof pdescr.tree === "object" ) {
                                    this.go_deep(entry_ky,pdescr)
                                }
     ...
     * @param {object} transformed 
     */
    lift_parameter_iterates(transformed) {
        for ( let [sk_name, op_map ] of Object.entries(transformed) ) {
            for ( let [op, skel ] of Object.entries(op_map) ) {
                let params_obj = skel.parameterized
                if ( params_obj ) {
                    for ( let [ky,pars] of Object.entries(skel.parameterized) ) {
                        let pararameter_ky = Object.keys(pars).find(el => {
                            if ( el.startsWith("_params<") ) return true
                            return false
                        })
                        if ( pararameter_ky ) {
                            let pars_list = pararameter_ky.replace("_params<","").replace(">","").split(',')
                            for ( let par of pars_list ) {
                                let par_descr = pars[par]
                                if ( par_descr?.recursive?._is_loop ) {
                                    // console.log("FOUND LOOP GENERATOR")
                                    // console.dir(par_descr.element_generator)
                                    // console.log("its data:",par_descr.data)
                                }
                            }
                        }
                    }
    ...
    /**
     * Expand the range expression under p_def.element_generator.generator to a list of numbers 
     * OR loop through range start to end.
     * 
     * p_def.data will contain the formula with the range variable in it. 
     * This replaces the range variable throughout the data and removes the header containing the 
     * variable parameter.
     * 
     * @param {object} p_def 
     * @returns {string}
     */
    coalesce_range(p_def) {
        let loop_var = p_def.element_generator.var_name
        //console.log("coalesce_range",p_def.element_generator.var_name)
        let range_def = p_def.element_generator.generator
        let range_pair = range_def.replace('<','').replace('>').split(',').map(el=> parseInt(el.trim()))
        let dform = p_def.data
        let expansion_to_return = ""
        if ( dform.length ) {
            let b_marker_str = `<@${loop_var}>`
            let beg_mark = dform.indexOf(b_marker_str)
            beg_mark += b_marker_str.length
            let end_marker = dform.indexOf(`</@${loop_var}>`)
            dform = dform.substring(beg_mark,end_marker)
            let replaced = `@{${loop_var}}`
            for ( let i = range_pair[0]; i <= range_pair[1]; i++ ) {
                let next_section = dform.replaceAll(replaced,i)
                expansion_to_return += next_section + "\n"
            }
        }

// console.log("coalesce_range")
// console.log(expansion_to_return)

        return expansion_to_return
    }



    coelesce_array_resolve_conditionals(var_name,replaced,value,next_section) {

        let section_parts = next_section.split('>>')

        if ( section_parts.length > 1 ) {
            section_parts.shift()
            section_parts = section_parts.map( sect => sect.substring(0,sect.lastIndexOf('<<')) )
            for ( let parseable of section_parts ) {
                let exec_report = {}
                if ( this.ternary_conditional(parseable,exec_report) ) {
//console.log("coelesce_array_resolve_conditionals",parseable)
//console.dir(exec_report)
                    if ( (replaced === exec_report.condition.cond) || (var_name === exec_report.condition.variable) ) {
                        if ( value.length ) {
                            next_section = next_section.replaceAll(`>>${parseable}<<`,exec_report.positive_exec.pos)
                        } else {
                            next_section = next_section.replaceAll(`>>${parseable}<<`,exec_report.negative_exec.neg)
                        }
                    }
                }
            }
    ...
    /**
     * 
     * @param {object} fetch_directive 
     * @param {object} params_def 
     * @returns {Array}
     */
    async fetch_element_list(fetch_directive,params_def) {
        switch ( fetch_directive.$_type ) {
            case "script"  : {
                // get output from a script -- use param defs to spec fields 
                let file_to_run = fetch_directive.data_source
                file_to_run = this.paths.compile_one_path(file_to_run)
                console.log(file_to_run)
                //
                let pars = Object.keys(params_def).filter(par => (par !== "_var_names")).join(',')
                let data_str = sys_utils.bash_command(file_to_run,pars)
    ...
    /**
     * JSON.parse the p_def.element_generator.generator expression and get an array of objects 
     * whose fields match the parameters in the parameter block of the data.
     * 
     * 
     * p_def.data will contain the formula with the element field variables in it as a parameter list.
     * This replaces the variables throughout the data and removes the header containing the 
     * variable parameter.
     * 
     * For each entry in the array, conditional evaluation is also done.
     * 
     * 
     * @param {object} p_def 
     * @returns {string}
     */
    async coalesce_array(p_def) {
        let loop_el_var = p_def.element_generator.var_name
        let list_data = p_def.element_generator.generator

        let el_list = JSON.parse(list_data)
        let did_fetch_test = false
        if ( !(Array.isArray(el_list)) ) {
            el_list = await this.fetch_element_list(el_list,p_def.recursive.params_def)
            did_fetch_test = true
        }
        let dform = p_def.data
        let expansion_to_return = ""
        if ( dform.length ) {
            let b_marker_str = `<@${loop_el_var}>`
            let end_marker_str = `</@${loop_el_var}>`
            let beg_mark = dform.indexOf(b_marker_str)
            beg_mark += b_marker_str.length
            let end_marker = dform.indexOf(end_marker_str)
            dform = dform.substring(beg_mark,end_marker)
            //
            let el_fields = Object.keys(p_def.recursive.params_def).filter(ky => (ky !== "_var_name"))
            //
            for ( let el of el_list ) {
                let next_section = "" + dform
                for ( let fld of el_fields ) {
                    let value = el[fld]
                    value = ((value === undefined) ? "" : value)
                    let replaced = `@{${fld}}`
                    if ( p_def.recursive?.executables?.execs?.length ) {
                        next_section = this.coelesce_array_resolve_conditionals(fld,replaced,value,next_section)
                    }
                    if ( value.length === 0 ) {
                        let subst_def = this.gen_var_allowed(fld)
                        if ( typeof subst_def === "string" ) {
                            value = subst_def
                        }
                    }
                    next_section = next_section.replaceAll(replaced,value)
                }
                expansion_to_return += next_section + "\n"
            }
        }
    ...
    up_prop_tree_values(p_def) {
        let looper = p_def.recursive?._is_loop
        if ( !looper ) {
            let params_def = p_def.recursive?.params_def
            if ( params_def ) {
                for ( let par in params_def ) {
                    let type = params_def[par]
                    if ( type === "%string%" ) {
                        params_def[par] = p_def.tree[par].data
                        if ( !params_def[par] ) {
                            params_def[par] = ""
                        }
                    } else if ( type.length ) {
                        let sub_p_def = params_def[par]
                        this.up_prop_tree_values(sub_p_def)
                        let pdescr = sub_p_def.tree[par]
                        this.executable_condition_processing(pdescr)
                        this.tree_conditional_reduction(pdescr)
                        pdescr.data = this.join_executables(pdescr)
     ...
     * @returns {string}
     */
    async data_from_parameter(skel,entry_ky,pname) {
...
// if ( entry_ky.indexOf("MAP-window_list.jsmplt") >= 0 ) {
// console.log("data_from_parameter",entry_ky)
// console.dir(p_def,{ depth: 4 })
// }

            if ( typeof data === "string" ) {
                if ( p_def.tree ) {     // built a parameter tree from nested parameters
                    let rec = p_def.recursive
                    if ( rec ) {
                        let dst_pars = rec.params_def
                        if ( dst_pars ) {
                            for ( let par in dst_pars ) {
                                if ( p_def.tree[par] === undefined ) {
                                    console.log("bad condition....","data_from_parameter",par,entry_ky)
//console.log("data_from_parameter",entry_ky)
//console.dir(p_def,{ depth: 4})
                                    continue
                                }
                                //
                                this.up_prop_tree_values(p_def.tree[par])
                                //
                ...
                } else if ( p_def?.recursive?._is_loop ) {              // interned objects describing loops, ranges and lists
// if ( "files::params::flexy_items_B-main.smplt" === entry_ky ) {
// console.log("data_FROM_PARAMETER",entry_ky)
// // debugger;
// console.trace()
// }
                    if ( p_def.element_generator?.is_range === true ) {              // RANGE <start,end>
                        will_return_data = this.coalesce_range(p_def)
                    } else if ( p_def.element_generator?.is_range === false ) { // a list of objects stored in an array
                        will_return_data = await this.coalesce_array(p_def)
                    } else {
                        will_return_data = p_def.data       // lied 
                    }
        ...
        // parameter block marked with @params ... @list does not contain this...
     ...
     * @param {object} transformed
     */
    async up_prop_data_to_vars(transformed) {
        for ( let [sk_name, op_map ] of Object.entries(transformed) ) {
            for ( let [op, skel ] of Object.entries(op_map) ) {
                let sk_map = skel.skeleton_map
                if ( typeof sk_map !== "object" ) continue
                for ( let [entry_ky,desciptor] of Object.entries(sk_map) ) {  // skeleton map
                    if ( desciptor?.recursive ) {
                        let rec = desciptor.recursive       // skeleton map version of recursive
                        desciptor.backup_data = "" + desciptor.data
                        if ( rec.params_def ) {     // paramerers 
                            //
                            for ( let [pname,ptype] of Object.entries(rec.params_def) ) {
                                if ( this.is_tree_type(ptype) ) {
                                    let pdat = await this.data_from_parameter(skel,entry_ky,pname)
            ...
            for ( let [op, skel ] of Object.entries(op_map) ) {
                ...
                for ( let [entry_ky,desciptor] of Object.entries(sk_map) ) {
                    if ( entry_ky.indexOf("script::") >= 0 ) {
                        ...
                        if ( entry_ky.indexOf("link::") >= 0 ) {
                            this.add_to_css_staging(desciptor)
                        }
                        // 
     ...
     * 
     */
    async run_css_staging() {
        for ( let ky in this.css_map_queue ) {
            let css_source = ky
            let css_dst = ky
            //
            if (css_source.indexOf('/') < 0 ) {
                css_source = `[alpha-css]/${css_source}`
            }
            if (css_dst.indexOf('/') < 0 ) {
                css_dst = `[template-css]/${css_dst}`
            }
            if ( css_source[0] === '.' ) {
                css_source = css_source.replace('..','[alpha-copious]')
            }
            if ( css_dst[0] === '.' ) {
                css_dst = css_dst.replace('..','[template-configs]')
            }
            css_source = this.paths.compile_one_path(css_source)
            css_dst = this.paths.compile_one_path(css_dst)

            console.log("RUN CSS STAGING",ky)
            console.log("RUN CSS STAGING",css_source, css_dst)
            await fos.ensure_directories(css_dst,"",true)
            await fos.copyFile(css_source,css_dst)
        }
    }


    extract_file_path(desciptor) {
        let desciptor_parts = desciptor.split("href='")
        let path_data = desciptor_parts[1]
        if ( path_data ) {
            path_data = path_data.substring(0,path_data.indexOf('.css') + 4)
            return path_data
        }
        return false
    }
     ...
     * 
     * @param {string} desciptor 
     */
    add_to_css_staging(desciptor) {
        if ( desciptor.indexOf('css') > 0 ) {
            //
            let extracted_descriptor =  this.extract_file_path(desciptor)
            if ( extracted_descriptor ) {
                let p = this.css_map_queue[extracted_descriptor]
                this.css_map_queue[extracted_descriptor] = p ? p + 1 : 1
            }
            //
        }
    }
...
//         await this.prep_script_directories(occurence_partition)
        ...
        this.lift_parameter_iterates(transform_1)

        this.list_to_skeletal_variable_assignment(transform_1)

        await this.up_prop_data_to_vars(transform_1)
    ...
    async load_file_or_leave_it_as_path(concern_replace) {
        ...
        if ( fname[0] === '\"' ) fname = fname.replaceAll('\"','')
    ...
    async output_ogroup_template(concerns,dir_form,tfile,template,concern_rewrites) {
        let promises = []
        for ( let concern of concerns ) {
            let c_template = "" + template  // each concern can change the template in its own way
            let top_out_dir = dir_form.replace("@concern",concern)
            let t_output_path = `${top_out_dir}${tfile}`
            let t_path = this.paths.compile_one_path(t_output_path)
            if ( concern_rewrites ) {
                for ( let rewrite_rule of Object.values(concern_rewrites) ) {  // each rewrite rule per concern
                    ...
                    if ( concern_replace ) {
                        ...
                        if ( c_template.indexOf(rewrite_rule.find) > 0 ) {
                            c_template = c_template.replaceAll(rewrite_rule.find,concern_replace)
                        } else if ( c_template.indexOf(rewrite_rule.alternate_find) > 0 ) {
                            c_template = c_template.replaceAll(rewrite_rule.alternate_find,concern_replace)
                        } else {
            ...
            promises.push(fos.write_out_string(t_path,c_template))
        }
        //
        await Promise.all(promises)
    }
    ...
    concern_match(op_key,concerns_list) {
        let match_concern = false
        for ( let concern of concerns_list ) {
            if ( op_key.indexOf(concern) > 0 ) {
                match_concern = concern
                break;
            }
        }
        return match_concern
    }
...
console.error(this.outputs)
        ...
        for ( let ogroup of outputs ) {
            ...
            let targets = ogroup.targets // targets
            let concerns_list = targets.concerns


            for ( let [tfile,file] of Object.entries(ogroup.saved_abbrev_skels) ) {
                //
                let skel_compiled_path = ogroup.skels_processed[file].ablsolute_path
                let op_key_list = ogroup.skels_processed[file].skel_vars_key
                if ( !(op_key_list) || (op_key_list.length === 0) ) {
                    op_key_list = ["defaults"]
                }
                //
                let op_keys_to_concerns = {}
                for ( let op_key of op_key_list ) {
                    let concern = this.concern_match(op_key,concerns_list)
                    if ( concern ) {
                        op_keys_to_concerns[op_key] = [concern]
                    } else {
                        op_keys_to_concerns[op_key] = [].concat(concerns_list)
                    }
                }


                for ( let op_key in op_keys_to_concerns ) {
                    let skel_data = parsed_skels[skel_compiled_path][op_key]  // This is the stuff for template generation

                    let concerns_for_key = op_keys_to_concerns[op_key]
...
//    console.dir(rewrites)
                    ...
                    // puts in quotes -- but maybe they are already there
                                    // generic_dir_spec = generic_dir_spec.replace('{{{ ','{{{"').replace(' }}}','"}}}')
    ...
    console.log("<2>generate_all_concerns_templates","concern_rewrites.....",generic_dir_spec, form)
                                    skel_data.concern_rewrites[ky] = {
                                        "find" : form,
                                        "alternate_find" : form.replaceAll('\"','\\\"'),
                    ...
                    await this.output_ogroup_template(concerns_for_key,dir_form,tfile,template,skel_data.concern_rewrites )
                }
            }
        }
console.log("COLLECTING CONCERNS")
        //
     ...
     *  "name" : "about",
        "content" : {
            "svg" : "", // the data for an `.svg` file will be stored here...
            "file" : "./about.svg"          // the file is located in the static directory
        }
     *
     ...
     * Provided by the user (admin).
     * 
     * @param {object} new_tracking -- newely generated by the tool
     * @param {object} saved_track_map -- saved by the user
     * @returns {boolean}
     */
    update_tracker_with_new(new_tracking,saved_track_map) {
        for ( let tky in new_tracking ) {
            if ( saved_track_map[tky] ) continue
            saved_track_map[tky] = new_tracking[tky]  // reflects that the tool may be updated with new presets
        }
    ...
    async load_ogroups_intermediate_files() {
        let ogroups_file = this.offset_from_project_dir(`ogroups_intermediate_files.json`)
     ...
     * 
     * `_calc.db` files are created for each page, so that a user can change them to provide access to prefered content.
     * An editor is supplied for editing the files in another project. Files with names like "index_calc.db" would be found in 
     * the [websites]/templates_config directory. A file such as "index_calc.db" would be for "index.html", while a file such as 
     * "login_calc.db" would be for "login.html".
     * 
    ...
    async write_templates_op_data() {
        ...
        //
        let concerns_file = this.offset_from_project_dir(`concerns_to_files.json`)
...
//                         promises.push(fos.write_out_string(t_path,template))
                        ...
                        if ( this.tracking_skel_calc_usage[sk] ) {
                            ...
                            // This means there is a tracker file
                            if ( tracker = await this.existing_calc_track(calc_tracking_file) ) {
                                ...
                                // tracker is now the one saved by the user, perhaps with some new informaton
                            }
    ...
    async skeleton_unification() {
        ...
        // at this point all_skeletons has the orginal copy of its text
        // further, for each operation the skeleton supports, the op mapping, has a "pure op" name key to global variable values
        // and also a "concern annotated op" name key to global variable values.
        // 

        await this.load_name_drops_db() // loads this db only -- this is a DB of operations referred to as calculators.
        //
